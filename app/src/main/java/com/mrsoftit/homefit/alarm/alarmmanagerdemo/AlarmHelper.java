package com.mrsoftit.homefit.alarm.alarmmanagerdemo;
...
import com.mrsoftit.homefit.receiver.BootReceiver;
...
public class AlarmHelper {
    ...
    private PendingIntent getPendingIntent() {
        Intent intent = new Intent("com.mrsoftit.homefit.alarmmanagerdemo.NOTIFY_ACTION");
        intent.setClass(this.context, NotificationPublisher.class);
        intent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
        return PendingIntent.getBroadcast(this.context, getNextRequestCode(), intent, PendingIntent.FLAG_IMMUTABLE);
    }

    private void setBootReceiverEnabled(int i) {
        Log.d(TAG, "setBootReceiverEnabled: ");
        this.context.getPackageManager().setComponentEnabledSetting(new ComponentName(this.context, BootReceiver.class), i, PackageManager.DONT_KILL_APP);
    }

    public PendingIntent existAlarm(int i) {
        Intent intent = new Intent("com.mrsoftit.homefit.alarmmanagerdemo.NOTIFY_ACTION");
        intent.setClass(this.context, NotificationPublisher.class);
        return PendingIntent.getBroadcast(this.context, i, intent, PendingIntent.FLAG_IMMUTABLE);
    }

    public boolean isAlarmScheduled(int i) {
        Intent intent = new Intent("com.mrsoftit.homefit.alarmmanagerdemo.NOTIFY_ACTION");
        intent.setClass(this.context, NotificationPublisher.class);
        return PendingIntent.getBroadcast(this.context, i, intent, PendingIntent.FLAG_IMMUTABLE) != null;
    }
